@extends('layouts.backoffice.master', ['title' => 'Halaman Order Barang'])
    ...
    <x-container>
        @role('admin')
            <div class="col-12">
                ...
                <div class="d-flex justify-content-end">{{ $orders->links('pagination::bootstrap-5') }}</div>
            </div>
        @endrole
        @role('customer')
                        ...
                        <tbody>
                            @foreach ($orders as $i => $order)
                                <tr>
                                    ...
                                    <td class="{{ $order->status == App\Enums\OrderStatus::Pending ? 'text-danger' : 'text-success'}}">
                                        {{ $order->status->value }}
                                    </td>
                                    <td>
                                        @if($order->status == App\Enums\OrderStatus::Pending)
