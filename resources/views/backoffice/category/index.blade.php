...
@section('content')
    <x-container>
        <div class="col-12 col-lg-8">
                        ...
                        @foreach ($categories as $i => $category)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @endcan
                                    @can('category-delete')
                                        <x-button-delete id="{{ $category->id }}" title="Hapus Data" url="{{ route('backoffice.category.destroy', $category->id) }}"/>
                                    @endcan
                                </td>
                            </tr>
                        @endforeach
            ...
            <div class="d-flex justify-content-end">{{ $categories->links('pagination::bootstrap-5') }}</div>
        </div>
        @can('category-create')
            <div class="col-12 col-lg-4">
                <form action="{{ route('backoffice.category.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <x-card title="Tambah Kategori" class="card-body">
                        <x-input title="Nama Kategori" name="name" type="text" placeholder="Masukan Nama Kategori" value="{{ old('name') }}" />
                        <x-input title="Gambar Kategori" name="image" type="file" placeholder="Pilih Gambar" value="{{ old('image') }}"/>
                        <x-button-save title="Simpan"/>
                    </x-card>
                </form>
            </div>
